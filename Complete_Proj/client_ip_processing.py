...
def main():
	...
	while 1:
		...
		if client_says.lower()=='exit':
			print("Coun: Bye")
			break
		
		if client_says.lower()=='hi':
			
			print("Coun: Hello, how are you doing today")
			continue
		#print(client_says)
	...
	#STOPWORDS REMOVAL
		...
		if toks in Mapping:
			...
			#for wei in Mapping[toks]:
			#	if Weights[toks][wei] < 0.2:
			#		eliza_needed=1
			eliza_needed=0
			...
			if training_mode==1:
				try:
					...
					if rating>10:
						user_resp=[]
						reply_user=raw_input("Enter your preferred response: ")
						#user_resp.append(reply_user)
						#ok=make_tuple(reply_user,10)
						###ok=tuple(user_resp.append(10))
					
					update_weight(Weights,toks,ok[1],rating)
					
		
		# Search for similar tuples
		else:
			for ma in Mapping:
				if Wordnet_sim(toks,ma) > 0.5:
					eliza_needed=0
		...
		if eliza_needed==1:
			...
			#with open("client.txt", "wb") as fo:
			#	fo.write(client_says)
			with open("./howie/frontends/client.txt", "wb") as fo:
				fo.write(client_says)
	#imp closing the file here..since writes to a file on close
			#fo.write(client_says)

			p=subprocess.call(['python','runme.py'])

			with open("foo.txt", "r") as outp:
				bot_says=outp.read()
				print ("Coun: "+bot_says)
